...
public class freserva {
   ...
   private conexion msyql=new conexion();
    ...
    sSQL="select r.idreserva,r.idhabitacion,h.numero,r.idcliente,"+
            ...
            " (select apaterno from persona where idpersona=r.idtrabajador)as trabajador,"+
        ...
        try {
            ...
            while (rs.next()){
                ...
                registro[4]=rs.getString("cliente") + " " + getString("clienteap");
                registro[5]=rs.getString("idtrabajador");
                registro[6]=rs.getString("trabajadorn");
                registro[7]=rs.getString("idcliente") + " " + rs.getString("trabajadorap");
                registro[8]=rs.getString("tipo_resera");
                registro[9]=rs.getString("fecha_resera");
    ...
    public boolean insertar (vreserva dts){
        sSQL="insert into reserva (Idhabitacion,Idcliente,Idtrabajador,Tipo_Reserva,Fecha_Reserva,Fecha_Ingreso,Fecha_Salida,Costo,Estado)" +
                "values (?,?,?,?,?,?,?,?,?)";
        try {
